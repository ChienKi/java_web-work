...
public class file_trans {
     ...
     * @param socket
     * @param outputDir 本地目录，需要加上接收到的文件名
     */
    public file_trans(Socket socket, String outputDir){
        mkdirsForParentFile(outputDir);
        try {
...
//            dis.close();
    ...
    public file_trans(String inputPath, Socket socket){
        try {
            ...
            File file = new File(inputPath);//           dos.writeUTF(file.getName());				//将文件名发送给服务器
    ...
    /**
     * (mkdirsForParentFile)如果文件目录不存在则创建目录
     * @param outputPath : 包含文件名的路径
     */
    public void mkdirsForParentFile(String outputPath){
